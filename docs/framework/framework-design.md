...
| `required_skills` | string[] | 本阶段必须加载的技能（引用 `.claude/skills/routing.yaml`） |
...
| `run` | Run | 绑定到该分支的执行实例 |
| `state_file` | string | `.claude/workflow-stage.yaml` 路径 |
...
Agent 通过 `scripts/lincoln-status.py` 读取当前阶段，从 `.claude/skills/routing.yaml` 获取推荐技能列表。
...
2. 读取 `.claude/workflow-stage.yaml`、`stage-manifest.yaml`、当前阶段 4 个上下文文件
...
flowchart TD
    ...
    Lincoln -->|读取| WS[".claude/workflow-stage.yaml<br/>当前阶段 / Run 状态"]
    Lincoln -->|读取| SM[".claude/stages/stage-manifest.yaml<br/>阶段元信息 / 上下文路径"]
    Lincoln -->|读取| SR[".claude/skills/routing.yaml<br/>阶段→技能映射"]
...
+----------------------------------+
| Lincoln 编排层                   |
|  - .claude/workflow-stage.yaml (状态)    |
|  - .claude/stages/stage-manifest.yaml (阶段)    |
|  - .claude/skills/routing.yaml (技能路由) |
+----------------------------------+
      |
